"""
2x2 grid run: 2 reward functions x 2 decision window pairs.
Inference runs once per window pair (prompts don't depend on reward params).
"""
...
DECISION_PAIRS = {
    "DP1_115_230": [115, 230],   # ~33% and ~66% of median call (344s)
    "DP2_90_180":  [90,  180],   # ~25% and ~50% of median
}
...
train_ids, val_ids, _, _ = train_test_split(
    train_ids, all_labels[:len(train_ids)], test_size=0.25,
    random_state=42,
    stratify=all_labels[:len(train_ids)])
...
def make_prompt(transcript, t):
    return (f"Below is the first {t} seconds of the negotiation between "
            f"the buyer (Speaker 0) and the seller (Speaker 1):\n"
            f"{transcript}\n"
            f"Will this negotiation end in a deal (respond with 'yes' or 'no'):  ")
...
# ── Main grid loop ─────────────────────────────────────────────────────────────
# Cache inference results per window pair (prompts don't change with reward fn)
inference_cache = {}   # dp_name -> {split: {conv_id: prob_yes per state}}
...
for dp_name, windows in DECISION_PAIRS.items():
    ...
    # Build transcripts for this window pair
    ...
    # Run inference once per window pair (reuse across reward functions)
    ...
    # Merge prob_yes back into split dataframes
    for split_name in ["val", "test"]:
        split_df = data[split_name]
        for m in [m1, m2]:
    ...
    # ROC-AUC (same for both reward functions)
    for split_name in ["val", "test"]:
        for m in [m1, m2]:
            auc = roc_auc_score(
                data[split_name]["is_sale"],
                data[split_name][f"prob_yes_{m}"])
            print(f"  ROC-AUC {split_name} {m}s: {auc:.3f}")

    # Now apply each reward function
    for rf_name, rf in REWARD_FUNCTIONS.items():
        cost, benefit = rf["cost"], rf["benefit"]
        print(f"\n  -- {rf_name} (cost={cost}, benefit={benefit}) --")

        # Compute optimal actions (for reporting action distribution)
        ...
        # Tune thresholds on val set (backward induction)
        N = 10000
        best_thr = {}
        ...
        auc_t45 = roc_auc_score(test_preds["is_sale"], test_preds[f"prob_yes_{m1}"])
        auc_t60 = roc_auc_score(test_preds["is_sale"], test_preds[f"prob_yes_{m2}"])
...
out_csv = os.path.join(os.path.dirname(__file__), "data", "results_grid_2x2.csv")
